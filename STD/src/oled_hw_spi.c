...
void OLED_Refresh_Gram(void)
{
    ...
    for (i = 0; i < 8; i++)
    {
        oled_write_operate(OLED_COMM, 0xb0 + i); //设置页地址(0--7)
        oled_write_operate(OLED_COMM, 0x00);     //设置显示位置—列低地址
        oled_write_operate(OLED_COMM, 0x10);     //设置显示位置—列高地址
...
void OLED_ShowNum1(u8 x, u8 y, u32 num, u8 len, u8 size)
{
    ...
    for (t = 0; t < len; t++)
    {
        ...
        if (enshow == 0 && t < (len - 1))
        {
            if (temp == 0)
            {
                OLED_ShowChar1(x + (size / 2) * t, y, ' ', size, 1);
                continue;
            }
            else
                enshow = 1;
        }
        OLED_ShowChar1(x + (size / 2) * t, y, temp + '0', size, 1);
    }
}
/*************************************************************************/
/*函数功能: 显示字符串*/
